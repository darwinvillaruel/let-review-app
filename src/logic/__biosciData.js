const biosci = [
  {
    id: 1,
    question: "Where do producers obtain their energy?",
    answers: [
      { text: "From other producers", correct: false },
      { text: "From the consumers", correct: false },
      { text: "From the decomposers", correct: false },
      { text: "From the sun", correct: true },
    ],
  },
  {
    id: 2,
    question:
      "Why do you experience not seeing things clearly for some seconds when you enter suddenly a well-lighted room after coming from a dark room?",
    answers: [
      { text: "The pupils are not adapted to the dark", correct: false },
      { text: "The eyes are adapted to the light only", correct: false },
      { text: "The pupils are not dilated yet", correct: true },
      { text: "Light had caused temporary blindness", correct: false },
    ],
  },
  {
    id: 3,
    question: "What term is used to describe the ability to maintain a constant internal environment?",
    answers: [
      { text: "Metabolism", correct: false },
      { text: "Growth and development", correct: false },
      { text: "Homeostasis", correct: true },
      { text: "Thermoregulation", correct: false },
    ],
  },
  {
    id: 4,
    question: "What is the most common cause of cardiovascular disease?",
    answers: [
      { text: "Fatty deposits in the arteries", correct: true },
      { text: "Inadequate supply of red blood cells", correct: false },
      { text: "Lack of sodium in the diet", correct: false },
      { text: "Increased heartbeat", correct: false },
    ],
  },
  {
    id: 5,
    question: "Which is the smallest unit of life that can survive and reproduce on its own?",
    answers: [
      { text: "Cell", correct: true },
      { text: "Organ", correct: false },
      { text: "Tissues", correct: false },
      { text: "Population", correct: false },
    ],
  },
  {
    id: 6,
    question:
      "Which region of a vertebrate forebrain is considered with the neural-endocrine control of visceral activities?",
    answers: [
      { text: "Cerebellum", correct: false },
      { text: "Thalamus", correct: false },
      { text: "Hypothalamus", correct: true },
      { text: "Pituitary", correct: false },
    ],
  },
  {
    id: 7,
    question: "Which of the following is the building block carbohydrate?",
    answers: [
      { text: "Amino acid", correct: false },
      { text: "Nucleotide", correct: false },
      { text: "Fatty acid", correct: false },
      { text: "Monosaccharide", correct: true },
    ],
  },
  {
    id: 8,
    question: "Why is it necessary for men to have more red blood cells per cubic millimeter of blood than women?",
    answers: [
      { text: "Man does strenuous activities and need more energy", correct: true },
      { text: "They have bigger physique and need more blood", correct: false },
      { text: "Man waste more blood cells and need extra", correct: false },
      { text: "They are more prone to anemia disorders", correct: false },
    ],
  },
  {
    id: 9,
    question: "Which of the following is the building block of protein?",
    answers: [
      { text: "Amino acid", correct: true },
      { text: "Nucleotide", correct: false },
      { text: "Fatty acid", correct: false },
      { text: "Monosaccharide", correct: false },
    ],
  },
  {
    id: 10,
    question: "Where does protein synthesis take place?",
    answers: [
      { text: "Ribosomes", correct: true },
      { text: "Lysosomes", correct: false },
      { text: "Endoplasmic reticulum", correct: false },
      { text: "Golgi bodies", correct: false },
    ],
  },
  {
    id: 11,
    question: "Why are our lips redder than our palm?",
    answers: [
      { text: "There is increased blood flow in the lips", correct: false },
      { text: "Lips are heavily keratinized", correct: false },
      { text: "Lip epidermis is thinner", correct: true },
      { text: "Many blood vessels are located around the lips", correct: false },
    ],
  },
  {
    id: 12,
    question: "Which of the following cannot be found in prokaryotes?",
    answers: [
      { text: "Cytoplasm", correct: false },
      { text: "Plasma membrane", correct: false },
      { text: "Membrane-bound nucleus", correct: true },
      { text: "DNA", correct: false },
    ],
  },
  {
    id: 13,
    question: "What do we call mutation that is brought about by the earth's natural radioactivity?",
    answers: [
      { text: "Continuous", correct: false },
      { text: "Induced", correct: false },
      { text: "Spontaneous", correct: true },
      { text: "Artificial", correct: false },
    ],
  },
  {
    id: 14,
    question:
      "Which of the following organelles produces turgor pressure against cell wall and mainly acts as water reservoir?",
    answers: [
      { text: "Plastids", correct: false },
      { text: "Vacuole", correct: true },
      { text: "Lysosomes", correct: false },
      { text: "Chloroplast", correct: false },
    ],
  },
  {
    id: 15,
    question: "Which microbial organism is most commonly used in the creation of genetically modified organisms?",
    answers: [
      { text: "Pseudomonas vulgaris", correct: false },
      { text: "Bacillus thuringensis", correct: false },
      { text: "Agrobacterium tumefaciens", correct: false },
      { text: "Escherichia coli", correct: true },
    ],
  },
  {
    id: 16,
    question: "What is the branch of zoology that is devoted to the study of fish?",
    answers: [
      { text: "Helminthology", correct: false },
      { text: "Ichthyology", correct: true },
      { text: "Chondrichthyes", correct: false },
      { text: "Ornithology", correct: false },
    ],
  },
  {
    id: 17,
    question: "The process involved in the production of mRNA using DNA as template is termed ________?",
    answers: [
      { text: "Transcription", correct: true },
      { text: "Replication", correct: false },
      { text: "Gene manipulation", correct: false },
      { text: "Reverse transcription", correct: false },
    ],
  },
  {
    id: 18,
    question: "What is the most abundant inorganic compound in the protoplasm?",
    answers: [
      { text: "Fluid", correct: false },
      { text: "Blood", correct: false },
      { text: "Plasma", correct: false },
      { text: "Water", correct: true },
    ],
  },
  {
    id: 19,
    question: "Which is the richest type of tropical rain forest in the Philippines?",
    answers: [
      { text: "Molave forest", correct: false },
      { text: "Dipterocarp forest", correct: true },
      { text: "Mangrove forest", correct: false },
      { text: "Pine forest", correct: false },
    ],
  },
  {
    id: 20,
    question: "What muscle is described as involuntary, not striated, and have a single nucleus?",
    answers: [
      { text: "Smooth", correct: true },
      { text: "Skeletal", correct: false },
      { text: "Cardiac", correct: false },
      { text: "Connective", correct: false },
    ],
  },
  {
    id: 21,
    question: "We feel warmer just before it rains because __________.",
    answers: [
      { text: "Heat is released by the evaporation of water", correct: false },
      { text: "The clouds prevent heat from escaping the earth", correct: false },
      { text: "There is an increased relative humidity", correct: false },
      { text: "Heat is released by the condensation of water vapor", correct: true },
    ],
  },
  {
    id: 22,
    question: "Which of the following is not considered as a reproductive cell?",
    answers: [
      { text: "Gamete", correct: false },
      { text: "Somatic cell", correct: true },
      { text: "Egg cell", correct: false },
      { text: "Sperm cell", correct: false },
    ],
  },
  {
    id: 23,
    question: "The cell wall of plant is made of _______.",
    answers: [
      { text: "Lipids", correct: false },
      { text: "Cellulose", correct: true },
      { text: "Protein", correct: false },
      { text: "Cell membrane", correct: false },
    ],
  },
  {
    id: 24,
    question: "Which of the following is the correct sequence of the level of organization from lowest to highest?",
    answers: [
      { text: "Cells-system-organs-tissues", correct: false },
      { text: "System-organs-tissues-cells", correct: false },
      { text: "Cells-tissues-organs-system", correct: true },
      { text: "Tissues-systems-cells-organs", correct: false },
    ],
  },
  {
    id: 25,
    question: "At metaphase of the meiotic division, the chromosomes are attached to the spindle fibers as ________.",
    answers: [
      { text: "Double chromatids", correct: false },
      { text: "Non-homologous pairs", correct: false },
      { text: "Single chromatid strands", correct: false },
      { text: "A tetrad of four chromatids", correct: true },
    ],
  },
  {
    id: 26,
    question: "Which of the following is an example of behavioral adaptation?",
    answers: [
      { text: "Thick green stems of the cactus plant", correct: false },
      { text: "Thick fur of the bear", correct: false },
      { text: "Shedding leaves during summer", correct: true },
      { text: "Presence of cuticle on the upper surface of the leaves", correct: false },
    ],
  },
  {
    id: 27,
    question: "The bipolar nature of the cell membrane is due to _______.",
    answers: [
      { text: "Presence of carriers", correct: false },
      { text: "Presence of oxygen", correct: false },
      { text: "Phospholipids bilayer", correct: true },
      { text: "Integral proteins", correct: false },
    ],
  },
  {
    id: 28,
    question: "Which of the following is made up of a diploid number of cells?",
    answers: [
      { text: "Zygote", correct: true },
      { text: "Sperm cell", correct: false },
      { text: "Egg cell", correct: false },
      { text: "Both B and C", correct: false },
    ],
  },
  {
    id: 29,
    question: "Which plant has an underground stem modified for reproduction?",
    answers: [
      { text: "Ginger", correct: false },
      { text: "Camote", correct: false },
      { text: "Tubers", correct: true },
      { text: "Strawberry", correct: false },
    ],
  },
  {
    id: 30,
    question: "Which of the following structures serve as a passageway of food and air?",
    answers: [
      { text: "Epiglottis", correct: false },
      { text: "Trachea", correct: false },
      { text: "Pharynx", correct: true },
      { text: "Larynx", correct: false },
    ],
  },
  {
    id: 31,
    question: "Which kind of relationship is exhibited by algae and fungi in lichen?",
    answers: [
      { text: "Commensalism", correct: false },
      { text: "Parasitism", correct: false },
      { text: "Symbiosis", correct: true },
      { text: "Competition", correct: false },
    ],
  },
  {
    id: 32,
    question: "What do you call the finger-like folds on the inner linings of the small intestine?",
    answers: [
      { text: "Villi", correct: true },
      { text: "Appendix", correct: false },
      { text: "Rugae", correct: false },
      { text: "Cecum", correct: false },
    ],
  },
  {
    id: 33,
    question: "Aside from ADP, what else is the end production of the dark-reaction phase of photosynthesis?",
    answers: [
      { text: "Carbon dioxide", correct: false },
      { text: "Sugar", correct: true },
      { text: "Carbon", correct: false },
      { text: "Starch", correct: false },
    ],
  },
  {
    id: 34,
    question: "What is that flap-like structure that prevents the food from going the wrong way during swallowing?",
    answers: [
      { text: "Epiglottis", correct: true },
      { text: "Larynx", correct: false },
      { text: "Esophagus", correct: false },
      { text: "Pharynx", correct: false },
    ],
  },
  {
    id: 35,
    question: "Which pigment is dominant in red algae?",
    answers: [
      { text: "Phycocyanin", correct: false },
      { text: "Fucoxanthin", correct: false },
      { text: "Chlorophyll", correct: false },
      { text: "Phycoerythrin", correct: true },
    ],
  },
  {
    id: 36,
    question: "What is the membrane that surrounds the lungs?",
    answers: [
      { text: "Pleura", correct: true },
      { text: "Meninges", correct: false },
      { text: "Peritoneum", correct: false },
      { text: "Pericardium", correct: false },
    ],
  },
  {
    id: 37,
    question: "What is that 3-carbon sugar formed during the dark reaction phase of photosynthesis?",
    answers: [
      { text: "ADP", correct: false },
      { text: "NADPH", correct: false },
      { text: "ATP", correct: false },
      { text: "PGAL", correct: true },
    ],
  },
  {
    id: 38,
    question: "What is the product of carbohydrate digestion?",
    answers: [
      { text: "Fatty acid", correct: false },
      { text: "Glucose", correct: true },
      { text: "Amino acid", correct: false },
      { text: "Nucleic acid", correct: false },
    ],
  },
  {
    id: 39,
    question: "What process is responsible for the upward movement of the water in very tall trees?",
    answers: [
      { text: "Osmosis", correct: false },
      { text: "Capillary action", correct: false },
      { text: "Turgor pressure", correct: false },
      { text: "Transpiration", correct: true },
    ],
  },
  {
    id: 40,
    question: "What is the enzyme in the mouth that breaks starch to maltose?",
    answers: [
      { text: "Peptidase", correct: false },
      { text: "Ptyalin", correct: true },
      { text: "Pepsin", correct: false },
      { text: "Maltase", correct: false },
    ],
  },
  {
    id: 41,
    question: "What do you call the types of symmetry where the body parts are paired on either side of the body?",
    answers: [
      { text: "Dorsal", correct: false },
      { text: "Bilateral", correct: true },
      { text: "Radial", correct: false },
      { text: "Ventral", correct: false },
    ],
  },
  {
    id: 42,
    question: "Which is considered as the respiratory center of the brain?",
    answers: [
      { text: "Cerebellum", correct: false },
      { text: "Medulla oblongata", correct: true },
      { text: "Cerebrum", correct: false },
      { text: "Thalamus", correct: false },
    ],
  },
  {
    id: 43,
    question: "Which stage of incomplete metamorphism is undergone by grasshopper?",
    answers: [
      { text: "Nymph, pupa, adult", correct: false },
      { text: "Egg, larva, pupa, adult", correct: false },
      { text: "Egg, nymph, adult", correct: true },
      { text: "Egg, pupa, adult", correct: false },
    ],
  },
  {
    id: 44,
    question: "Which of the following is NOT part of the thoracic cage?",
    answers: [
      { text: "Clavicle", correct: true },
      { text: "Ribs", correct: false },
      { text: "Costal cartilages", correct: false },
      { text: "Sternum", correct: false },
    ],
  },
  {
    id: 45,
    question: "Where does the toxin substance secreted by bees and ants through their sting originate?",
    answers: [
      { text: "Silk glands", correct: false },
      { text: "Intestines", correct: false },
      { text: "Rectal glands", correct: true },
      { text: "Salivary glands", correct: false },
    ],
  },
  {
    id: 46,
    question: "What is the longest bone in the body?",
    answers: [
      { text: "Humerus", correct: false },
      { text: "Femur", correct: true },
      { text: "Tibia-fibula", correct: false },
      { text: "Clavicle", correct: false },
    ],
  },
  {
    id: 47,
    question: "Which causes our bones to turn brittle and easily break?",
    answers: [
      { text: "Turning into muscle tissues", correct: false },
      { text: "Increase in flexibility", correct: false },
      { text: "Turbidity decrease", correct: false },
      { text: "Removal of collagen", correct: true },
    ],
  },
  {
    id: 48,
    question: "How many bones does an adult human body have?",
    answers: [
      { text: "201", correct: false },
      { text: "200", correct: false },
      { text: "217", correct: false },
      { text: "206", correct: true },
    ],
  },
  {
    id: 49,
    question: "What do you call automatic responses to an external stimulus?",
    answers: [
      { text: "Reflex", correct: true },
      { text: "Instinct", correct: false },
      { text: "Impulse", correct: false },
      { text: "All of these", correct: false },
    ],
  },
  {
    id: 50,
    question: "What structure provides flexible support and protection to the spinal cord?",
    answers: [
      { text: "Skull", correct: false },
      { text: "Ribs", correct: false },
      { text: "Vertebral column", correct: true },
      { text: "Sternum", correct: false },
    ],
  },
  {
    id: 51,
    question: "Which hormone is insufficient in cretinism?",
    answers: [
      { text: "Glucagon", correct: false },
      { text: "Adrenalin", correct: false },
      { text: "Insulin", correct: false },
      { text: "Thyroxine", correct: true },
    ],
  },
  {
    id: 52,
    question: "Which of the following is both endocrine and an exocrine gland?",
    answers: [
      { text: "Gastric glands", correct: false },
      { text: "Thyroid glands", correct: false },
      { text: "Pituitary glands", correct: false },
      { text: "Pancreas", correct: true },
    ],
  },
  {
    id: 53,
    question: "What organ is controlled by the automatic nervous system?",
    answers: [
      { text: "Skeletal", correct: false },
      { text: "Muscles", correct: false },
      { text: "Heart", correct: true },
      { text: "Both A and B", correct: false },
    ],
  },
  {
    id: 54,
    question: "What do you call food in a semi-liquid form, partially digested and mixed with hydrochloric acid?",
    answers: [
      { text: "Gastric secretion", correct: false },
      { text: "Gastric fluid", correct: false },
      { text: "Mucosa", correct: false },
      { text: "Chyme", correct: true },
    ],
  },
  {
    id: 55,
    question: "Which is considered the master gland due to its influence on the activity of all the other glands?",
    answers: [
      { text: "Thyroid", correct: false },
      { text: "Adrenal", correct: false },
      { text: "Pituitary", correct: true },
      { text: "Parathyroid", correct: false },
    ],
  },
  {
    id: 56,
    question: "Which part of the brain is responsible for intelligence, memory and learned behavior?",
    answers: [
      { text: "Cerebellum", correct: false },
      { text: "Cerebrum", correct: true },
      { text: "Medulla", correct: false },
      { text: "Thalamus", correct: false },
    ],
  },
  {
    id: 57,
    question: "Which of these materials is not a major component of the plasma membrane?",
    answers: [
      { text: "Phospholipids", correct: false },
      { text: "Glycoprotein", correct: false },
      { text: "Proteins", correct: false },
      { text: "DNA", correct: true },
    ],
  },
  {
    id: 58,
    question: "Which form of RNA delivers information from DNA to be used in making protein?",
    answers: [
      { text: "Messenger RNA", correct: true },
      { text: "Ribosomal RNA", correct: false },
      { text: "Transfer RNA", correct: false },
      { text: "All of the above", correct: false },
    ],
  },
  {
    id: 59,
    question: "Which of the following is not a type of cell?",
    answers: [
      { text: "Bacterium", correct: false },
      { text: "Amoeba", correct: false },
      { text: "Sperm", correct: false },
      { text: "Virus", correct: true },
    ],
  },
  {
    id: 60,
    question:
      "Which property of water is probably MOST important for the functioning of organisms at the molecular level?",
    answers: [
      { text: "Cohesion and high surface tension", correct: false },
      { text: "High specific heat", correct: false },
      { text: "High heat of vaporization", correct: false },
      { text: "Versatility as a solvent", correct: true },
    ],
  },
  {
    id: 61,
    question: "Which of the following statements is true of diffusion?",
    answers: [
      { text: "It requires an expenditure of energy by the cell", correct: false },
      { text: "It is a passive process", correct: true },
      {
        text: "It occurs when molecules move from a region of lower concentration to one of higher concentration",
        correct: false,
      },
      { text: "It requires a membrane", correct: false },
    ],
  },
  {
    id: 62,
    question: "Which of the following is FALSE in comparing prophase I of meiosis and prophase of mitosis?",
    answers: [
      { text: "The chromosomes condense in both", correct: false },
      { text: "Tetrads form in both", correct: true },
      { text: "The nuclear envelope disassembles in both", correct: false },
      { text: "A spindle forms in both", correct: false },
    ],
  },
  {
    id: 63,
    question: "Which are small, irregularly-shaped cells without nuclei but are rich in ATP?",
    answers: [
      { text: "Leukocytes", correct: false },
      { text: "Thrombocytes", correct: false },
      { text: "Lymphocytes", correct: false },
      { text: "Erythrocytes", correct: true },
    ],
  },
  {
    id: 64,
    question:
      "What branch of Biology is involved when one is studying the characteristic structure and functions of all kinds of cells?",
    answers: [
      { text: "Cytology", correct: true },
      { text: "Morphology", correct: false },
      { text: "Physiology", correct: false },
      { text: "Histology", correct: false },
    ],
  },
  {
    id: 65,
    question: "Which part of seed appears as a scar which marks the entry of the pollen tube during fertilization?",
    answers: [
      { text: "Seed leaf", correct: false },
      { text: "Cotyledon", correct: false },
      { text: "Testa", correct: false },
      { text: "Hilum", correct: true },
    ],
  },
  {
    id: 66,
    question: "Which type of tissue is responsible for the secondary growth in stems and roots of vascular plants?",
    answers: [
      { text: "Meristematic cells", correct: false },
      { text: "Cambium", correct: true },
      { text: "Palisade layer", correct: false },
      { text: "Epidermis", correct: false },
    ],
  },
  {
    id: 67,
    question: "Which biome grows tropical grassland with scattered individual trees and larger herbivores?",
    answers: [
      { text: "Tundra", correct: false },
      { text: "Rain forest", correct: false },
      { text: "Grassland", correct: false },
      { text: "Savanna", correct: true },
    ],
  },
  {
    id: 68,
    question: "What do you call that process in birds wherein they shed off features at least once a year?",
    answers: [
      { text: "Skinning", correct: false },
      { text: "Molting", correct: true },
      { text: "Furrowing", correct: false },
      { text: "Migrating", correct: false },
    ],
  },
  {
    id: 69,
    question: "What chemical controls metamorphosis among insects?",
    answers: [
      { text: "Enzymes", correct: true },
      { text: "Estradol", correct: false },
      { text: "Pheromones", correct: false },
      { text: "Ethylene", correct: false },
    ],
  },
  {
    id: 70,
    question: "Which of the following makes up the plant cell wall?",
    answers: [
      { text: "Nucleic acid", correct: false },
      { text: "Cellulose", correct: true },
      { text: "Protein", correct: false },
      { text: "Lipids", correct: false },
    ],
  },
  {
    id: 71,
    question: "Which of the following is not true of fungi?",
    answers: [
      { text: "Some are pathogenic", correct: false },
      { text: "Some are photosynthetic", correct: true },
      { text: "Some are edible", correct: false },
      { text: "Form symbiotic relation with algae", correct: false },
    ],
  },
  {
    id: 72,
    question: "Which of the following is not true of plants?",
    answers: [
      { text: "Some are pathogenic", correct: false },
      { text: "Some are photosynthetic", correct: false },
      { text: "Some are edible", correct: false },
      { text: "Form symbiotic relation with algae", correct: true },
    ],
  },
  {
    id: 73,
    question: "Which of the following is considered multicellular?",
    answers: [
      { text: "Embryo", correct: true },
      { text: "Fertilized egg", correct: false },
      { text: "Egg cell", correct: false },
      { text: "Sperm cell", correct: false },
    ],
  },
  {
    id: 74,
    question: "What hormone promotes the growth of the uterine lining for the implantation of the embryo?",
    answers: [
      { text: "Lactogenic hormone", correct: false },
      { text: "Progesterone", correct: true },
      { text: "Testosterone", correct: false },
      { text: "Oxytocin", correct: false },
    ],
  },
  {
    id: 75,
    question: "Which of the following pair of organisms are closely related?",
    answers: [
      { text: "Spider and mosquito", correct: false },
      { text: "Frog and crocodile", correct: false },
      { text: "Man and monkey", correct: true },
      { text: "Ants and aphids", correct: false },
    ],
  },
  {
    id: 76,
    question: "What does it mean if the blood pressure of human is 110/70?",
    answers: [
      { text: "The systolic pressure is 70", correct: false },
      { text: "The diastolic pressure is 70", correct: true },
      { text: "The pulse rate is 110 beats per minute", correct: false },
      { text: "Both B and C", correct: false },
    ],
  },
  {
    id: 77,
    question: "Which of the following happens when man exhales?",
    answers: [
      { text: "The residual volume of the lungs decreases", correct: false },
      { text: "The diaphragm contracts", correct: false },
      { text: "The volume of the thoracic cavity decreases", correct: true },
      { text: "The lung contracts", correct: false },
    ],
  },
  {
    id: 78,
    question: "Which of the following regulate breathing?",
    answers: [
      { text: "RBC concentration", correct: false },
      { text: "Number of hemoglobin", correct: false },
      { text: "Hormone level in blood", correct: false },
      { text: "CO2 and O2 concentration and pH level sensors", correct: true },
    ],
  },
  {
    id: 79,
    question: "Which of the following has the lowest velocity of blood flow?",
    answers: [
      { text: "Veins", correct: true },
      { text: "Arteries", correct: false },
      { text: "Capillaries", correct: false },
      { text: "Arterioles", correct: false },
    ],
  },
  {
    id: 80,
    question: "How many carbon atoms can each pyruvic acid supply into the Kreb’s cycle?",
    answers: [
      { text: "8", correct: false },
      { text: "6", correct: false },
      { text: "4", correct: false },
      { text: "2", correct: true },
    ],
  },
  {
    id: 81,
    question:
      "How many oxygen is required each time a molecule of glucose is completely oxidized through aerobic respiration?",
    answers: [
      { text: "3", correct: false },
      { text: "6", correct: true },
      { text: "12", correct: false },
      { text: "24", correct: false },
    ],
  },
  {
    id: 82,
    question: "Which of the following will not be accepted by glycolysis for catabolism?",
    answers: [
      { text: "Starch", correct: false },
      { text: "Glycerol", correct: false },
      { text: "Fatty acid", correct: true },
      { text: "Sucrose", correct: false },
    ],
  },
  {
    id: 83,
    question: "Which of the following characteristic is common between echinoderms and cnidarians?",
    answers: [
      { text: "They are both radically symmetrical", correct: true },
      { text: "They have segmented bodies", correct: false },
      { text: "Both have stinging cells", correct: false },
      { text: "They have three embryonic tissue layers", correct: false },
    ],
  },
  {
    id: 84,
    question: "Which of the following is true of Phylum Chordata?",
    answers: [
      { text: "Presence of notochord that provides skeletal support", correct: false },
      { text: "Pharyngeal slits that have become modified for gas exchange", correct: false },
      { text: "Dorsal hollow nerve cord that develops into the central nervous system", correct: false },
      { text: "I, II and III", correct: true },
    ],
  },
  {
    id: 85,
    question: "Which of the following cannot be associated with fungi?",
    answers: [
      { text: "Absorptive nutrition", correct: false },
      { text: "Decomposers", correct: false },
      { text: "Autotrophs", correct: true },
      { text: "Spore production", correct: false },
    ],
  },
  {
    id: 86,
    question: "Which of the following does not involve mitosis?",
    answers: [
      { text: "Development of embryo", correct: false },
      { text: "Growth", correct: false },
      { text: "Production of gametes", correct: true },
      { text: "Repairing of damaged tissues", correct: false },
    ],
  },
  {
    id: 87,
    question: `Which of the following is true of mammals?
    I. They have four chambered heart
    II. They produce milk for the young
    III. They are warm blooded`,
    answers: [
      { text: "I and II", correct: false },
      { text: "II and III", correct: false },
      { text: "I and III", correct: false },
      { text: "I, II and III", correct: true },
    ],
  },
  {
    id: 88,
    question: "What term describes an egg developing into a new organism without itself being fertilized by a sperm?",
    answers: [
      { text: "Regeneration", correct: false },
      { text: "Conjugation", correct: false },
      { text: "Parthenogenesis", correct: true },
      { text: "Fusion", correct: false },
    ],
  },
  {
    id: 89,
    question: "Which of the following is responsible for formation of blood cells?",
    answers: [
      { text: "Cartilage", correct: false },
      { text: "Bone marrow", correct: true },
      { text: "Joints", correct: false },
      { text: "Tendons", correct: false },
    ],
  },
  {
    id: 90,
    question: "What is most likely to happen when a species cannot adapt to the changes in the environment?",
    answers: [
      { text: "It will be transformed to another form", correct: false },
      { text: "It will become extinct", correct: true },
      { text: "It will be isolated", correct: false },
      { text: "It will grow old", correct: false },
    ],
  },
  {
    id: 91,
    question: "What term is used to describe the release of a matured egg cell from the ovary?",
    answers: [
      { text: "Fertilization", correct: false },
      { text: "Copulation", correct: false },
      { text: "Ovulation", correct: true },
      { text: "Germination", correct: false },
    ],
  },
  {
    id: 92,
    question: "Which of the following blood cells is responsible for blood clotting?",
    answers: [
      { text: "Erythrocytes", correct: false },
      { text: "Leukocytes", correct: false },
      { text: "Thrombocytes", correct: true },
      { text: "Neutrophils", correct: false },
    ],
  },
  {
    id: 93,
    question: "Where does ectopic pregnancy occur?",
    answers: [
      { text: "Uterus", correct: false },
      { text: "Ovary", correct: false },
      { text: "Cervix", correct: false },
      { text: "Fallopian tube", correct: true },
    ],
  },
  {
    id: 94,
    question: "Why is cell membrane semi-permeability important?",
    answers: [
      { text: "It is important in determining the size of the cell", correct: false },
      { text: "It controls the kind of substances that enters and leaves the cell", correct: true },
      { text: "It controls the pressure and energy gradient", correct: false },
      { text: "It allows the molecules to diffuse freely in and out of the cell", correct: false },
    ],
  },
  {
    id: 95,
    question:
      "What theory of evolution accounts for snake’s disappearance of legs and development of giraffe’s long neck?",
    answers: [
      { text: "Theory of use and disuse", correct: true },
      { text: "Theory of natural selection", correct: false },
      { text: "Theory of chromosomal change", correct: false },
      { text: "Theory of action and interaction", correct: false },
    ],
  },
  {
    id: 96,
    question: "Which of the following is an exocrine gland?",
    answers: [
      { text: "Plastids and pigments", correct: false },
      { text: "Asters and centrioles", correct: true },
      { text: "Chloroplasts and vacuoles", correct: false },
      { text: "Cell wall and cell membrane", correct: false },
    ],
  },
  {
    id: 97,
    question: "Which of the following is an exocrine gland?",
    answers: [
      { text: "Thyroid gland", correct: false },
      { text: "Salivary gland", correct: true },
      { text: "Pituitary gland", correct: false },
      { text: "Adrenal gland", correct: false },
    ],
  },
  {
    id: 98,
    question: "Which of the following is not an accessory organ of digestion?",
    answers: [
      { text: "Liver", correct: false },
      { text: "Pancreas", correct: false },
      { text: "Salivary gland", correct: false },
      { text: "Adrenal gland", correct: true },
    ],
  },
  {
    id: 99,
    question: "What hormone stimulates the development of male secondary sex characteristics?",
    answers: [
      { text: "Estrogen", correct: false },
      { text: "Prolactin", correct: false },
      { text: "Testosterone", correct: true },
      { text: "Progesterone", correct: false },
    ],
  },
  {
    id: 100,
    question: "Which of the following parts performs both reproductive and urinary functions in males?",
    answers: [
      { text: "Ureter", correct: false },
      { text: "Testes", correct: false },
      { text: "Urethra", correct: true },
      { text: "Scrotum", correct: false },
    ],
  },
];

export default biosci;
