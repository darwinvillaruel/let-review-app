...
const GenEdSpecial = () => {
  ...
  const choice = Number(params.get("questions")) || 100;
  ...
  const random = randomizedGenEd
    .filter((item) => item.tag === "actual")
    .slice(0, choice);

  const question = random[currentIndex];

  useEffect(() => console.log(random), [question]);
